...
const getTemperatureInfo = (setTemperatures, setParsedData) => {
    Promise.allSettled([
...
const getRunningProcesses = (setRunningProcesses, setParsedData) => {
    Promise.allSettled([
        ...
        if (!allData.runningProcesses) {
            setParsedData(result);
            return;
        }

        const processText = allData.runningProcesses.replace(/^"|\s*"$/g, "").trim();
        ...
        setParsedData(result);
        console.log("Parsed Running Processes:", result);
    });
};

const getSecurityInformation = (setSecurityInformation, setParsedData) => {
    ...
    Promise.allSettled([
        ...
        results.forEach((result, index) => {
            if (result.status === "fulfilled") {
                ...
                console.error(`Security command ${index} failed:`, result.reason);
            }
        });
...
const getManualInstallsInfo = (setManualInstalls, setDirectoryStructure, setCurrentPath, setAvailableOptions, setCurrentFiles, setParsedData) => {
    Promise.allSettled([
        runCommand("list_manual_installs", []).then((output) => {
            ...
            return { "type": "manualInstalls", "value": output };
        }),
    ]).then((results) => {
        results.forEach((result, index) => {
            if (result.status === "fulfilled") {
